...
def generate_fornax(infile,basename,n_subbands,rts_templates,array,options):
    ...
    if (options.auto_gen):
        ...
        rts_file = open('qRTS_auto_wrapper_%d.sh' % options.chunk_number,'w+')
    ...
    #        rts_file.write('./RTS_cal_up.py --list=%s\n' % infile)
        ...
        rts_file = open('qRTS_auto_inner_%d.sh' % options.chunk_number,'w+')
        for line in in_file:
            obs_id = (line.split())[0]
