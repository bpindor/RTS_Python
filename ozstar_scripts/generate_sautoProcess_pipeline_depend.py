...
#        out_file.write('echo $QA_JOB1\n')
        out_file.write('QA_JOB3=$(sbatch  --array=1-%s --dependency=afterok:$RTS_JOB_%d qQA_pipeline_phase_amp_plots.sh | cut -d " " -f 4)\n' % (n_obs,options.chunk_number))
        out_file.write('echo $QA_JOB3\n')
        out_file.write('QA_JOB2=$(sbatch  --array=1-%s --dependency=afterany:$RTS_JOB_%d sQA_pipeline_plot_rms.sh | cut -d " " -f 4)\n')
        out_file.write('echo $QA_JOB2\n')
        out_file.write('LAST_JOB=$QA_JOB2\n')   
